 ...
 *******************************************************************************/
package langhua.ofbiz.webapp.jarscan;
...
public class CasJarScanner implements JarScanner {
	private static final String module = CasJarScanner.class.getName();
	...
	static {
		// Debug.logInfo("============== CasJarScanner ==============", module);
		Set<ClassLoader> cls = new HashSet<>();

		ClassLoader cl = CasJarScanner.class.getClassLoader();
		while (cl != null) {
			cls.add(cl);
			cl = cl.getParent();
		}
	 ...
	 */
	private JarScanFilter jarScanFilter = new CasJarScanFilter();
	...
	public void scan(JarScanType scanType, ServletContext context, JarScannerCallback callback) {
		...
		// Debug.logInfo("[" + scanType + "] " + sm.getString("jarScan.webinflibStart"), module);
		...
		if (dirList != null) {
			for (String path : dirList) {
				if (path.endsWith(Constants.JAR_EXT)
					...
					// Debug.logInfo("[" + scanType + "] " + sm.getString("jarScan.webinflibJarScan", path), module);
					URL url = null;
					try {
					...
					// Debug.logInfo("[" + scanType + "] " + sm.getString("jarScan.webinflibJarNoScan", path), module);
				}
			}
		}
	...
	protected void doScanClassPath(JarScanType scanType, ServletContext context, JarScannerCallback callback,
		...
		// Debug.logInfo("[" + scanType + "] " + sm.getString("jarScan.classloaderStart"), module);
	...
	protected void processURLs(JarScanType scanType, JarScannerCallback callback, Set<URL> processedURLs,
		...
		while (!classPathUrlsToProcess.isEmpty()) {
			...
			if ((cpe.isJar() || scanType == JarScanType.PLUGGABILITY || isScanAllDirectories())
				...
				// Debug.logInfo("[" + scanType + "] " + sm.getString("jarScan.classloaderJarScan", url), module);
				try {
					processedURLs.add(url);
				...
				// Debug.logInfo("[" + scanType + "] " + sm.getString("jarScan.classloaderJarNoScan", url), module);
			}
		}
	}
	...
	protected void process(JarScanType scanType, JarScannerCallback callback, URL url, String webappPath,
		...
		// Debug.logInfo("[" + scanType + "] " + sm.getString("jarScan.jarUrlStart", url), module);
    ...
    private static void processManifest(Jar jar, boolean isWebapp, Deque<URL> classPathUrlsToProcess)
        ...
        if (manifest != null) {
            ...
            Debug.logInfo("--Scanning manifest of " + jarURL.getPath() + "--", module);
            ...
            for (String classPathEntry : classPathEntries) {
                ...
                Debug.logInfo("-- Manifest class path --" + classPathEntry, module);
